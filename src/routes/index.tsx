...
const catalogImages = [
  "/images/ARH-131-client-reference.webp",
...
function Standards() {
  ...
  const cameraImages = [hero, angle, detail, lifestyleDark];
  ...
  return <section id="system" className="section-pad page-shell">
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <div className="fine-grid relative aspect-square overflow-hidden border bg-card p-[12%]"><div className="absolute inset-x-[15%] top-1/2 border-t border-signal/70"/><div className="absolute inset-y-[12%] left-1/2 border-l border-signal/70"/><img src={hero} alt="RF-A1592 sideboard centered within square composition guides" loading="lazy" width={512} height={768} className="size-full object-contain"/><span className="eyebrow absolute left-4 top-4">1:1 canvas / 70% product scale</span><span className="eyebrow absolute bottom-4 right-4 text-signal">10–15% breathing space</span></div>
      <div className="grid grid-cols-3 gap-3 self-end">{[[reference,"Too tight","p-0 scale-125"],[angle,"Inconsistent","p-[22%]"],[hero,"Balanced","p-[10%]"]].map(([src,label,fit],i)=><figure key={label} className={i===2?"border border-signal p-2":"border p-2 opacity-55"}><div className="aspect-square overflow-hidden bg-card"><img src={src} alt={`${label} framing example`} loading="lazy" width={512} height={512} className={`size-full object-contain ${fit}`}/></div><figcaption className="eyebrow mt-3">{label}</figcaption></figure>)}<p className="col-span-3 mt-3 text-2xl">Same scale. Same breathing room. One visual rhythm.</p></div>
    </div>
    <div className="mt-24 grid gap-8 border-t pt-8 lg:grid-cols-[1fr_2fr]"><div><p className="eyebrow text-muted-foreground">Camera standard</p><h3 className="display-title mt-4 text-5xl">Angles should explain the product.</h3><div className="mt-8 flex flex-wrap gap-2">{["Front","30–35°","Detail","In context"].map((name,i)=><Button key={name} size="sm" variant={camera===i?"primary":"outline"} onClick={()=>setCamera(i)}>{name}</Button>)}</div></div><div className="relative aspect-[4/3] overflow-hidden bg-card"><img src={cameraImages[camera]} alt={`RF-A1592 ${cameraNames[camera]} product view`} loading="lazy" width={512} height={768} className="size-full object-contain transition-all duration-500"/><span className="absolute bottom-4 left-4 bg-primary px-3 py-2 text-[10px] uppercase text-primary-foreground">{camera===1?"Preferred when depth matters":"Controlled camera view"}</span></div></div>
  </section>;
}
...
function SixSequence() {
  ...
  return <section id="sequence" className="relative bg-primary text-primary-foreground"><div className="sticky top-24 z-20 ml-auto hidden w-16 flex-col items-center gap-3 border-y border-primary-foreground/20 bg-primary/80 py-4 backdrop-blur md:flex">{sequence.map((s,i)=><a href={`#image-${i+1}`} key={s[0]} className={`text-[10px] transition-all ${active===i?"scale-125 text-accent":"opacity-35"}`}>{s[0]}</a>)}</div>
    {sequence.map((item,i)=><article key={item[0]} id={`image-${i+1}`} data-sequence data-index={i} className={`sequence-panel grid min-h-screen items-center gap-10 px-4 py-28 md:px-12 lg:grid-cols-[.8fr_1.2fr] ${active===i?"is-active":""}`}><div className="mx-auto max-w-xl"><p className="eyebrow text-accent">{item[0]} — {item[1]}</p><h2 className="display-title mt-6 text-6xl sm:text-7xl lg:text-8xl">{item[2]}</h2><p className="mt-7 max-w-md text-sm leading-7 text-primary-foreground/65">{item[3]}</p></div><div className="mx-auto aspect-square w-full max-w-3xl overflow-hidden bg-card"><img src={images[i]} alt={`RF-A1592 sideboard — ${imageNames[i]}`} loading="lazy" width={512} height={768} className="size-full object-contain"/></div></article>)}
  </section>;
}
